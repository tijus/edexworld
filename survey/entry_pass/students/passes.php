...
$sql_query = "SELECT * FROM entry_pass_students";
...
while ($row_query = mysqli_fetch_assoc($res_query)) {
    $id = $row_query['id'];
    $name = $row_query['name'];
    $father_name = $row_query['father_name'];
    $school = $row_query['school'];
    $standard = $row_query['standard'];
    ...
    if (floor($count / 10) % 2 == 0) {
        if (floor($count / 5) % 2 == 0) {
            ...
            $pdf->Cell(85, 6, '"MAKING OF A TOP SCORER CHILD"', 1, 2, 'C', 'true');
            $pdf->SetFont('Arial', 'B', 10);

            $pdf->SetFillColor(255, 255, 255);
            $pdf->Cell(85, 5, 'ID: ' . $id, 'LRT', 2, 'L', 'true');
            $pdf->Cell(85, 5, 'Name: ' . $name, 'LR', 2, 'L', 'true');
            $pdf->Cell(85, 5, 'Father\'s Name: ' . $father_name, 'LR', 2, 'L', 'true');
            $pdf->Cell(85, 5, 'School: ' . $school . '     Standard:' . $standard, 'LR', 2, 'L', 'true');
            $pdf->Cell(85, 5, 'Society: ' . $society, 'LR', 2, 'L', 'true');
